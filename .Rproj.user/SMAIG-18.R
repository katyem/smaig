...
SMAIGtable <- read.xlsx("SMAIG.xlsx", 1)  # read first sheet of your deck of stacks
#head(SMAIGtable[, 1:5])
...
#Repopulates the cubeCoord matrix with SMAIGTable data for the stackID passed
   cubeCoord <- tableStack(stackID=0) # if stackID==0, choose first stack in SMAIGtable
...
# This "randomly" repopulates the cubeCoord matrix variable
  ...
  displayStack(cMarker = T) #cMarker: blue for first cube and red for last cube
  #OR
  displayStack(newScreen = TRUE) # new device screen
  
# move focus between two or more screens up or down
...
#build mirror in cleared screen  (saveStack must be TRUE if you want to save the mirror)
  cubeCoord = mirrorStack(cMarker = TRUE, saveStack = TRUE)
# arguments:
...
# save picture as a png to your working directory. WARNING: overwrites
  ...
  i = 1 # should use the stackID in the SMAIGtable to match pictures to data
  snapName <- paste0("smaig", formatC(i, digits = 2, flag = "00"), ".png")
  rgl.snapshot(snapName)
  setwd("D:/R stuff/AIG") # Change to reflect your working directory

  
  
# READ the stack from the SMAIGtable
#Repopulates the cubeCoord matrix with SMAIGtable data for the stackID passed
  cubeCoord <- tableStack(stackID=7) 
