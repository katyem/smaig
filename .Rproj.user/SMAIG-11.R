...
cubeCoord <- matrix(
  ...
  nrow = stackSize+1,
  dimnames = list(
    c("one", "two", "three", "four","five","six","seven","eight","nine","ten","rotate"),
...
buildStack <- function(upLeg = 4, stackSize=10, cubeCoo = cubeCoord)  {
  ...
  # Final row: set up rotation x y z
  cubeCoo[stackSize+1,1] <- 99
  for (i in 2:4) {
    cubeCoo[stackSize+1,i] <- (sample(1:35, 1)*10)
  }
  return(cubeCoo)
}
...
cubeCoord <- tableStack(stackID=5) #Repopulates the cubeCoord matrix with the ID passed

displayStack(stackSize, spinStack = TRUE)
rgl.viewpoint(-120, 70,); 
