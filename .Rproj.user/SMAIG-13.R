...
saveStack <- function(cubeCoo = cubeCoord)  {
  ...
  x = nrow(cubeCoo) # should be 13; stackSize plus 3x3 userMatrix
  for (i in 1:x) {
    smRow = nrow(SMAIGnew)+1
    SMAIGnew[smRow,] = list(stackID+1,cubeCoo[i,1:4]) 
  }
  return(SMAIGnew)
}
...
#---------------------------------------------------------------------------------
#build the stack in cubeC using the SMAIGTable data frame which can have multiple stacks via stackID
...
displayStack <- function(stackSize=10, spinStack=TRUE){
...
cubeCoord <- tableStack(stackID=8) #Repopulates the cubeCoord matrix with the ID passed

displayStack(stackSize, spinStack = TRUE)
#rgl.viewpoint(); 

axes3d(color = 'white')  # add some axes

clear3d() #initiate/clear RGL screen
mirrorStack(x=cubeCoord,stackSize) 

# NEXT write function to store what is on the screen
