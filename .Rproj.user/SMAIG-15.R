...
buildStack <- function(upLeg = 4, stackSize=10)  {
  ...
  cubeCoo[11,2:4] <- newRotation[1:3]
  #print(cubeCoo)
  return(cubeCoo)
}
...
displayStack <- function(stackSize=10){
  ...
  for (i in 1:stackSize) { # build four legs using 'stackSize' number of cubes
    ...
    newMatrix = rotate3d(par3d("userMatrix"), cubeCoord[11,1], 1,0,0) # Rotate about model's x axis
    newMatrix = rotate3d(newMatrix, cubeCoord[11,2], 0,1,0) # Rotate about model's y axis
    newMatrix = rotate3d(newMatrix, cubeCoord[11,3], 0,0,1) # Rotate about model's z axis
...
storeMatrix <- function(){
  cubeCo <- cubeCoord
  degLatt = rglToLattice(rotm = round(par3d("userMatrix"),3))
  
  for (i in 1:3) {
    cubeCo[11,i+1] = round(degLatt[[i]][1],3) # FYI, pi == 180 degrees
  }
