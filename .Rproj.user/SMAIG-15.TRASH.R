...
#install.packages("orientlib")  # use rglToLattice(rotm = par3d("userMatrix")) to calculate xyz of userMatrix
...
blankMatrix <- function(stackSize=10) {
  ...
  nrow = stackSize+1,
  dimnames = list(
    c("one", "two", "three", "four","five","six","seven","eight","nine","ten","degree"),
...
buildStack <- function(upLeg = 4, stackSize=10)  {
  ...
  # Final row: set up rotation of stack x = theta in x y z
  dim(diag(4))
  ...
  myMatrix = diag(1,4,4) # diag = 1  
  i = 2.2
  myMatrix
  myMatrix = rotate3d(newMatrix, i, 1,0,0)  
  #myMatrix = rotate3d(newMatrix, i, 0,1,0)  
  #myMatrix = rotate3d(newMatrix, i, 0,0,1)   

  
  myMatrix
  par3d(userMatrix = myMatrix)
  degBase = rglToBase(rotm = myMatrix) 

  xTheta =  degBase[[1]][1] 
  xMatrix = 
 newMatrix = rotate3d(myMatrix, i, 1,0,0)
 
 newMatrix <- matrix(c(cos(xTheta),-sin(xTheta),0,sin(xTheta),cos(xTheta),0,0,0,1),3,3)
 newMatrix*myMatrix[1:3][1:3]
 #  oldMatrix = par3d('userMatrix')
 par3d(userMatrix = oldMatrix) 
 
  degBase = rglToBase()  #defaults to userMatrix
  ...
  newMatrix = newMatrix, degLatt[[1]][1], 1,0,0)
  newMatrix = rotate3d(newMatrix, degLatt[[2]][1], 0,1,0)
  newMatrix = rotate3d(newMatrix, degLatt[[3]][1], 0,0,1)
...
displayStack <- function(stackSize=10){
  ...
  newMatrix = rotate3d(newMatrix, cubeCoord[11,2], 1,1,1) #theta
  par3d(userMatrix = newMatrix)
