...
## Build stack - tried to use this as a function by passing upLeg and stackSize but couldn't get it to work i.e., no change in matrix
## update: used -- cubeCoord = buildStack() -- and it worked
buildStack <- function(upLeg = 4, stackSize=10, cubeCoo = cubeCoord)  {
  # clearStack 
  prevFace <- array(c(0,0))
  ...
  for (i in 1:stackSize) { # build four legs using 'stackSize' number of cubes
    ...
    if (legCubeCount == 1 && i > 1) {  # first cube is preset
        prevFace[2] <- face 
          while (prevFace[1] == face || prevFace[2] == face) {
            face <- sample(2:4, 1) # select face/axis 2=x 3=y 4=z
          }
        prevFace[1] <- prevFace[2]
...
#-----------------------------------------------------------------------------------
                   #Build the Stack!!
#-----------------------------------------------------------------------------------
cubeCoord = buildStack(cubeCoo=cubeCoord) # This "randomly" repopulates the cubeCoord matrix variable
